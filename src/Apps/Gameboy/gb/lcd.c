...
//
    ...
    // llapi_disp_put_point(nx + 48,ny,c);
	gfb_set_pixel(screen_frame, nx + 48, ny, c);

	//if(pfm % 2)
	//{
	//	if(!(y % 2))llapi_disp_put_point(nx,ny,c);
	//}else{
	//	if(y % 2)llapi_disp_put_point(nx,ny,c);
	//}
...
static inline int sprites_update(int line, struct sprite *spr)
{
	int i, c = 0;
...
static inline void sprite_fetch(int line, struct oam_cache *o)
{
...
static inline void lcd_do_line(int line, int cycle)
{
